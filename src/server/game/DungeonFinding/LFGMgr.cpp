...
namespace lfg
{
	...
	*/
	void LFGMgr::InitializeLockedDungeons(Player* player, uint8 level /* = 0 */)
	{
		...
		for (LfgDungeonSet::const_iterator it = dungeons.begin(); it != dungeons.end(); ++it)
		{
			...
			//sLog->outInfo(LOG_FILTER_SERVER_LOADING,"%u %u %u %u %u",dungeon->Entry(),dungeon->minlevel,dungeon->maxlevel,level); //Debug Line
	...
	*/
	void LFGMgr::FinishDungeon(uint64 gguid, const uint32 dungeonId)
	{
		...
		{
		sLog->outError(LOG_FILTER_LFG, "GetDungeon(gguid) != dungeonId");
		sLog->outError(LOG_FILTER_LFG, "LFGMgr::FinishDungeon: [" UI64FMTD "](gguid) Finished dungeon %u(dungeonID) but group queued for %u(gDungeonId). Ignoring", gguid, dungeonId, gDungeonId);
