...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        std::string m_lastCombatCaster;
        uint32 m_lastCombatspell;
        ...
        // Max Rating
        uint32 GetMaxPersonalArenaRate() { return _maxPersonalArenaRate; }
        void SetMaxPersonalArenaRate(uint32 newRating) { _maxPersonalArenaRate = newRating; }

        PlayerCurrenciesMap &GetCurrenciesMap() { return _currencyStorage; }
        ...
        void RemovePet(Pet* pet, PetSlot mode, bool returnreagent = false, bool logout = false);
        ...
        */
        ...
        bool HasTwoHandWeaponEquipped() const
        {
            Item* mainItem = GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND);
            Item* offItem = GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND);
            return (offItem && offItem->GetTemplate()->InventoryType == INVTYPE_2HWEAPON)
                || (mainItem && mainItem->GetTemplate()->InventoryType == INVTYPE_2HWEAPON);
        }
        ...
        void CheckLevelDependantSpells();
        ...
        void DeleteArenaTeam(uint8 slot, bool update = false);
        static uint32 GetArenaTeamIdFromDB(uint64 guid, uint8 slot);
        static void LeaveAllArenaTeams(uint64 guid);
        uint32 GetArenaTeamId(uint8 slot) const { return GetUInt32Value(PLAYER_FIELD_ARENA_TEAM_INFO_1_1 + (_arenaTeamInfoSlot[slot] * ARENA_TEAM_END) + ARENA_TEAM_ID); }
        ...
        uint32 m_forced_speed_changes[MAX_MOVE_TYPE];
        uint32 m_movement_ack[ACK_TYPE_MAX];
        ...
        void SetAuraUpdateMaskForRaid(uint32 slot) { m_auraRaidUpdateMask |= (uint64(1) << slot); }
        ...
        */
        ...
        bool IsWatchingCinematic() const { return m_IsWatchingCinematic; }
        void SetWatchingCinematic(bool state) { m_IsWatchingCinematic = state; }
        uint32 m_holyPowerRegenTimerCount;

        PetData* GetPetDatabySlot(uint8 slot = 0);
        PetData* GetPetDatabySpell(uint32 spell);
        PetData* GetPetDatabyGuid(uint32 guid);
        PetData* GetPetDatabyEntry(uint32 entry);
    ...
    protected:
        ...
        */
        ...
        void _LoadPets(SQLQueryHolder *holder);
