...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature)
	{
		...
		pPlayer->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101181, 0, 0, pCreature);
		...
		void MoveInLineOfSight(Unit* who)
		{
			...
			else
			{
				if (me->GetDistance(who) > 15.0f)
					return;
				if (kick == true)
					who->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 100964, 0, 0, me);
			}
		}
...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature)
	{
		...
		pPlayer->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 98102, 0, 0, pCreature);
			...
			if (who->ToPlayer()->GetQuestStatus(29147) == QUEST_STATUS_INCOMPLETE)
			{
				if (Aura * aura = who->GetAura(98027))
				{
					...
					if (aura->GetStackAmount() > 64)
						who->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 100957, 0, 0, me);
				}
				else
		...
		void UpdateAI(const uint32 diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_TWO:
					if (Unit *pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0))
					{
						pTarget->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE, 52649, 0, 0, me);
				...
				case EVENT_TWO:
					...
					if (HealthBelowPct(5))
					{
						if (Unit *pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0))
						{
							...
							pTarget->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 100979, 0, 0, me);
						}
...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature)
	{
		...
		{
			if (pPlayer->GetCurrency(416, true) >= 20)
				return false;
...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature)
	{
		...
		{
			if (pPlayer->GetCurrency(416, true) >= 125)
				return false;
...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature)
	{
		pPlayer->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101176, 0, 0, pCreature);
		return false;
	}
...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature)
	{
		pPlayer->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101174, 0, 0, pCreature);
		return false;
	}
			...
			if (!PlayerList.isEmpty())
				for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
					if (i->getSource()->isAlive())
						if (me->GetExactDist(i->getSource()) < 50.0f)
							if (i->getSource())
								i->getSource()->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101167, 0, 0, me);
		}
...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature)
	{
		if (pCreature->GetEntry() == 52921)
			pPlayer->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101179, 0, 0, pCreature);
		else if (pCreature->GetEntry() == 53259)
			pPlayer->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101177, 0, 0, pCreature);
		else if (pCreature->GetEntry() == 52824)
			pPlayer->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101182, 0, 0, pCreature);
		else if (pCreature->GetEntry() == 52134)
			pPlayer->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101173, 0, 0, pCreature);
		else if (pCreature->GetEntry() == 52986)
			pPlayer->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101170, 0, 0, pCreature);
		else if (pCreature->GetEntry() == 52669)
		{
			pPlayer->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101175, 0, 0, pCreature);
					...
					for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
						if (i->getSource()->isAlive())
							if (me->GetExactDist(i->getSource()) < 100.0f)
								i->getSource()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 100992, 0, 0, me);
			}
		}
						...
						for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
							if (i->getSource()->isAlive())
							{
								if (me->GetExactDist(i->getSource()) > 10.0f)
									i->getSource()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 100996, 0, 0, me);
							}
...
public:
	...
	struct npc_flamewaker_sentinelAI : public ScriptedAI
	{
		...
		{
			if (me->HasAura(98169))
				killer->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE, 53085, 0, 0, me);
		}
					...
					if (i->getSource()->isAlive())
						if (me->GetExactDist(i->getSource()) < 120.0f)
						{
							if (me->GetEntry() == 54252)
								i->getSource()->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101098, 0, 0, me);
							if (me->GetEntry() == 54253)
								i->getSource()->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101097, 0, 0, me);
							if (me->GetEntry() == 54254)
								i->getSource()->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101099, 0, 0, me);
							if (me->GetEntry() == 54255)
								i->getSource()->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101100, 0, 0, me);
							if (me->GetEntry() == 54256)
								i->getSource()->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101101, 0, 0, me);
							if (me->GetEntry() == 54257)
								i->getSource()->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 101103, 0, 0, me);
						}
		}
